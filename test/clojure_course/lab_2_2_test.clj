...
(deftest time-test
  (testing "testing time optimization"
    ...
    (println (time ((integral sqr) 19.2)))
    (println)
    (println "sqr with partial sum integral:")
    (println (time ((integral-partial-sums sqr) 1)))
    (println (time ((integral-partial-sums sqr) 1)))
    (println (time ((integral-partial-sums sqr) 2)))
    (println (time ((integral-partial-sums sqr) 2)))
    (println (time ((integral-partial-sums sqr) 19.2)))
    (println (time ((integral-partial-sums sqr) 19.2)))))